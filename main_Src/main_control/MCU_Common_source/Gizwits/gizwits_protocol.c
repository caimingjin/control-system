...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagAIR_Power)
    {
        info->event[info->num] = EVENT_AIR_Power;
        info->num++;
        dataPoints->valueAIR_Power = gizStandardDecompressionValue(AIR_Power_BYTEOFFSET,AIR_Power_BITOFFSET,AIR_Power_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagLight_Power)
    {
        info->event[info->num] = EVENT_Light_Power;
        info->num++;
        dataPoints->valueLight_Power = gizStandardDecompressionValue(Light_Power_BYTEOFFSET,Light_Power_BITOFFSET,Light_Power_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagTV_Power)
    {
        info->event[info->num] = EVENT_TV_Power;
        info->num++;
        dataPoints->valueTV_Power = gizStandardDecompressionValue(TV_Power_BYTEOFFSET,TV_Power_BITOFFSET,TV_Power_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagAIR_Model)
    {
        info->event[info->num] = EVENT_AIR_Model;
        info->num++;
        dataPoints->valueAIR_Model = gizStandardDecompressionValue(AIR_Model_BYTEOFFSET,AIR_Model_BITOFFSET,AIR_Model_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
        
    if(0x01 == issuedData->attrFlags.flagAIR_temper)
    {
        info->event[info->num] = EVENT_AIR_temper;
        info->num++;
        dataPoints->valueAIR_temper = gizX2Y(AIR_temper_RATIO,  AIR_temper_ADDITION, issuedData->attrVals.valueAIR_temper); 
    }
        
    if(0x01 == issuedData->attrFlags.flagLight_level)
    {
        info->event[info->num] = EVENT_Light_level;
        info->num++;
        dataPoints->valueLight_level = gizX2Y(Light_level_RATIO,  Light_level_ADDITION, issuedData->attrVals.valueLight_level); 
    }
        
    if(0x01 == issuedData->attrFlags.flagTV_Channel)
    {
        info->event[info->num] = EVENT_TV_Channel;
        info->num++;
        dataPoints->valueTV_Channel = gizX2Y(TV_Channel_RATIO,  TV_Channel_ADDITION, issuedData->attrVals.valueTV_Channel); 
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueAIR_Power != cur->valueAIR_Power)
    {
        GIZWITS_LOG("valueAIR_Power Changed\n");
        ret = 1;
    }
    if(last->valueLight_Power != cur->valueLight_Power)
    {
        GIZWITS_LOG("valueLight_Power Changed\n");
        ret = 1;
    }
    if(last->valueTV_Power != cur->valueTV_Power)
    {
        GIZWITS_LOG("valueTV_Power Changed\n");
        ret = 1;
    }
    if(last->valueAIR_Model != cur->valueAIR_Model)
    {
        GIZWITS_LOG("valueAIR_Model Changed\n");
        ret = 1;
    }
    if(last->valueAIR_temper != cur->valueAIR_temper)
    {
        GIZWITS_LOG("valueAIR_temper Changed\n");
        ret = 1;
    }
    ...
    if(last->valueTV_Channel != cur->valueTV_Channel)
    {
        GIZWITS_LOG("valueTV_Channel Changed\n");
        ret = 1;
    }

    if(last->valueLight_Num != cur->valueLight_Num)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueLight_Num Changed\n");
            ret = 1;
        }
    }
    if(last->valueTV_NOW_Channel != cur->valueTV_NOW_Channel)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueTV_NOW_Channel Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(AIR_Power_BYTEOFFSET,AIR_Power_BITOFFSET,AIR_Power_LEN,(uint8_t *)devStatusPtr,dataPoints->valueAIR_Power);
    gizStandardCompressValue(Light_Power_BYTEOFFSET,Light_Power_BITOFFSET,Light_Power_LEN,(uint8_t *)devStatusPtr,dataPoints->valueLight_Power);
    gizStandardCompressValue(TV_Power_BYTEOFFSET,TV_Power_BITOFFSET,TV_Power_LEN,(uint8_t *)devStatusPtr,dataPoints->valueTV_Power);
    if(dataPoints->valueAIR_Model >= AIR_Model_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valueAIR_Model Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(AIR_Model_BYTEOFFSET,AIR_Model_BITOFFSET,AIR_Model_LEN,(uint8_t *)devStatusPtr,dataPoints->valueAIR_Model);  
    }
    gizByteOrderExchange((uint8_t *)devStatusPtr->wBitBuf,sizeof(devStatusPtr->wBitBuf));

    devStatusPtr->valueAIR_temper = gizY2X(AIR_temper_RATIO,  AIR_temper_ADDITION, dataPoints->valueAIR_temper); 
    devStatusPtr->valueLight_level = gizY2X(Light_level_RATIO,  Light_level_ADDITION, dataPoints->valueLight_level); 
    devStatusPtr->valueTV_Channel = gizY2X(TV_Channel_RATIO,  TV_Channel_ADDITION, dataPoints->valueTV_Channel); 
    devStatusPtr->valueLight_Num = gizY2X(Light_Num_RATIO,  Light_Num_ADDITION, dataPoints->valueLight_Num); 
    devStatusPtr->valueTV_NOW_Channel = gizY2X(TV_NOW_Channel_RATIO,  TV_NOW_Channel_ADDITION, dataPoints->valueTV_NOW_Channel); 
